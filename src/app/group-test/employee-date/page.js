...
export default function PayrollManagement() {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-6">
            ...
            {/* Data Table */}
            <div id="payrollwrapper" ref={componentRef} className="p-6 bg-white border rounded-lg shadow-md">
                {loading ? (
                    ...
                    <div className="overflow-auto">
                        {Object.entries(
                            payrollData.reduce((acc, record) => {
                                const empNo = record.employee?.EmpNo || "Unknown";
                                const payrollDate = record.payroll_date.split("T")[0]; // Extract the date part

                                if (!acc[empNo]) acc[empNo] = {}; // Initialize a nested object for each employee
                                if (!acc[empNo][payrollDate]) acc[empNo][payrollDate] = []; // Initialize an array for each date within an employee

                                acc[empNo][payrollDate].push(record); // Add the record to the appropriate employee-date group
                                return acc;
                            }, {})
                        ).map(([empNo, dateGroups]) => (
                            <div key={empNo} className="mb-6">
                                {/* Employee Header */}
                                <h1 className="text-lg font-bold text-center mb-4">
                                    Employee: {Object.values(dateGroups)[0][0]?.employee?.Firstname}{" "}
                                    {Object.values(dateGroups)[0][0]?.employee?.Surname} - Employee No: {empNo}
                                </h1>
                                {Object.entries(dateGroups).map(([payrollDate, records]) => (
                                    <div key={payrollDate} className="mb-6">
                                        {/* Date Header */}
                                        <h2 className="text-md font-semibold text-center mb-2">Date: {payrollDate}</h2>
                                        {/* Payroll Table */}
                                        <table className="w-full table-auto bg-white text-xs border border-gray-300">
                                            <thead className="bg-gray-100 text-gray-600">
                                                <tr>
                                                    <th className="px-2 py-2 border border-gray-300">Payroll ID</th>
                                                    <th className="px-2 py-2 border border-gray-300">Work Days</th>
                                                    <th className="px-2 py-2 border border-gray-300">Per Day Salary</th>
                                                    <th className="px-2 py-2 border border-gray-300">Total Basic</th>
                                                    <th className="px-2 py-2 border border-gray-300">Net Salary</th>
                                            ...
                                            <tbody>
                                                {records.map((record) => {
                                                    ...
                                                    const netSalary =
                                                        totalBasic - record.advance - record.festival_advance - record.loan_amount;
                                                    return (
                                                        <tr key={record.id} className="text-center border border-gray-300">
                                                            <td className="px-2 py-2 border border-gray-300">{record.id}</td>
                                                            <td className="px-2 py-2 border border-gray-300">{record.work_days}</td>
                                                            <td className="px-2 py-2 border border-gray-300">{record.per_day_salary}</td>
                                                            <td className="px-2 py-2 border border-gray-300">{totalBasic}</td>
                                                            <td className="px-2 py-2 border border-gray-300">{netSalary}</td>
                                                        </tr>
                                                    );
                                                })}
                                            </tbody>
