    ...
    const fetchPayrollData = async () => {
        ...
        try {
            const response = await fetch(
                `/api/payroll/groupedpayroll?page=${currentPage}&recordsPerPage=${recordsPerPage}&${new URLSearchParams(
                    searchParams
                )}`
            );
                        ...
                        <table className="w-full table-auto bg-white text-xs border border-gray-300 ">
                            ...
                            <tbody>
                                {payrollData.length > 0 &&
                                    payrollData.reduce((acc, record, index) => {
                                        const currentDate = record.payroll_date.split("T")[0];
                                        const previousDate = index > 0 ? payrollData[index - 1].payroll_date.split("T")[0] : null;

                                        // Add a new header if the payroll_date changes
                                        if (currentDate !== previousDate) {
                                            acc.push(
                                                <tr key={`header-${currentDate}`} className="bg-gray-200">
                                                    <td colSpan="100%" className="px-2 py-2 font-bold text-center border border-gray-300">
                                                        Payroll Date: {currentDate}
                                                    </td>
                                                </tr>
                                            );
                                        }
                                        ...
                                        acc.push(
                                            ...
                                            <tr key={record.id + Math.random()} onDoubleClick={() => handleviewascol(record)} className={`text-center focus:bg-slate-100 cursor-pointer ${selectededrow === record.id ? 'bg-slate-200' : ""} border border-gray-300`}
                                                onClick={() => setselectedrow(record.id)}>
                                                {/* Employee Table Fields */}
                                                <td className="px-2 py-2 border border-gray-300">{record.Nic_Passport || "-"}</td>
                                                <td className="px-2 py-2 whitespace-nowrap overflow-hidden text-ellipsis border border-gray-300">{record.Firstname + " " + record.Surname || "-"}</td>
                                                <td className="px-2 py-2 border border-gray-300">{record.Initials || "-"}</td>
                                                <td className="px-2 py-2 border border-gray-300">{record.EmpNo || "-"}</td>
                                                <td className="px-2 py-2 border border-gray-300">{record.EpfNo || "-"}</td>
